...
const std::string INPUT_FILE_NAME = "/storage/epp2/phrsnt/lblpwgtools/standardCAF/StateFilesAllSystematicsSplitBySign.root";

void sensitivityFullEstimate();
...
void sensitivityFullEstimate(int N_THROWS, int N_TEST_DELTA_CP_VALUES)
{
  std::string outputFileName = "/storage/epp2/phrsnt/lblpwgtools/standardCAF/fullEstimate/IZZLESensitivityPlotsFullEstimate_";

  outputFileName += std::to_string(N_THROWS);
  outputFileName += "THROWS";
  outputFileName += std::to_string(N_TEST_DELTA_CP_VALUES);
  outputFileName += "TESTVALUES";
  outputFileName += "_NO_SplitBySign.root";
  ...
  for (int j = 0; j < nTestCPValues; ++j)
  {
    //const double trueDeltaCP(static_cast<float>(j) * stepSizeCP);
	  ...
	  predictionVector = Get_Throw_NO(predictionGenerators, trueDeltaCP, pot);
  ...
  const bool isHigherOctant, const bool isPositiveHierarchy, bool fitCPC, double &bestChiSquared, std::map<std::string, float> &bestFitPosition)
{
  ...
  chiSquared = fit.Fit(calc, ana::IFitter::kQuiet)->EvalMetricVal();
