...
const std::string INPUT_FILE_NAME = "/storage/epp2/phrsnt/lblpwgtools/realRecoCAF/energySystematics/StateFilesEnergySystematicsSplitBySign.root";

void sensitivityFitEnergySystematicsTemp(int systematicsMode);
...
void sensitivityFitEnergySystematicsTemp(int systematicsMode)
{
  ...
  std::string outputFileName = "/storage/epp2/phrsnt/lblpwgtools/realRecoCAF/energySystematics/IZZLESensitivityPlotsEnergySystematicThrowsPlus_NO_SplitBySign.root";
  ...
  //outputFileName += "/IZZLESpectraPlotsSystematics_NO_SplitBySign_MX.root";
    ...
    if (MAKE_THROWS)
    {
      ...
      for (const ISyst * const systematic : systematics)
      {
        ...
        for (std::string &sysToShiftName : systematicsToShift)
        {
	      ...
	      systematicShifts.SetShift(systematic, std::fabs(GetBoundedGausThrowThis(systematic->Min() * 0.8, systematic->Max() * 0.8)));
	    }
	}
      }
    }
